...
export async function getChallenges(fid: number): Promise<IMatch[] | null> {
    try {
        const { data, error } = await supabase.from("match").select("*").eq("opponent_fid", fid);
...
export async function createChallenge(challenger_fid: number, challenger_fname: string, opponent_fid: number, opponent_fname: string): Promise<IMatch[] | null> {
...
export async function updateChallenge(id: number, challenger_score: number, opponent_score: number): Promise<IMatch | null> {
    const { data, error } = await supabase.from("match").update({ challenger_score, opponent_score }).eq("id", id).select();
    if (error) throw error;
    return data ? data[0] as IMatch : null;
}

export async function startOpponentWrestle(id: number): Promise<IMatch | null> {
    const { data, error } = await supabase.from("match").update({ opponent_start_time: new Date() }).eq("id", id).select();
    if (error) throw error;
    return data ? data[0] as IMatch : null;
}